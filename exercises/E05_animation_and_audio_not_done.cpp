...
static void game_update(SDLContext* context, GameState* state)
{
	// player
	{
		...
		if (velocity.x < 0 && !data->facing_left) {
			data->facing_left = true;
			entity->transform.scale.x *= -1;
		}
		else if (velocity.x > 0 && data->facing_left) {
		...
		// Animation
		SDL_Log("%f\n", velocity.x);
		float movement_threshold = 0.2f;
		if (SDL_fabsf(velocity.x) > movement_threshold && data->is_grounded) {
			// Walking
			float time_per_frame = 0.1f; // seconds per frame
			int num_frames = 8; // number of frames in the walking animation
			int frame = (int)(context->uptime / time_per_frame) % num_frames;
			entity->sprite.rect = SDL_FRect { (float)frame*96, 512, 96, 128 };
		}
			...
			// Idle
			entity->sprite.rect = SDL_FRect { 0, 0, 96, 128 };
		}
